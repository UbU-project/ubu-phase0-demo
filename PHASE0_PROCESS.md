...
1. The schema is frozen after the first ticket. Schema changes after freeze are the most expensive change in this architecture. Everything that might be needed later is given a schema slot now.
...
The demo must be impressive because it is **concrete**, not because it is broad. The primary demonstration value is association introspection: UbU using its own planning model to coordinate the UbU project in public.
...
- Audience-created issues are pulled in by the operator typing `refresh`. No polling, no background service.

### 2.4 Data sources

- Live source: `UbU-dummy/ubu-design` GitHub repository, accessed via an environment-variable token.
- Fixture source: a single canonical file `fixtures/static_dummy_issue.json`, which serves simultaneously as the test fixture and the offline corpus.
- No persistent on-disk cache. `refresh` replaces in-memory state for the running process only.
- Mode is selected by the operator: live by default, fixture via `--offline`. A live-mode error fails visibly and exits non-zero; it does not silently fall back.
...
- The schema includes `LogEntry`, `SkeletonFailureDiagnostic`, and `PlanCandidate.semi_legitimization_summary` even though v1 may not exercise them.
...
- Dependencies via `depends-on: #N` labels for live issues; hardcoded in `static_dummy_issue.json` for fixture mode.
- Missing `depends-on` target: warn, treat the dependency as absent, log the event, do not crash.
...
Allowed: `fixtures/static_dummy_issue.json`, `fixtures/demo_request.json`, `fixtures/demo_log_empty.json`, `fixtures/claim_register.json`, `tests/test_contract_roundtrip.py`.
...
**PHASE0-T007 — loop**
Allowed: `src/ubu_phase0/loop.py`, `tests/test_missing_github_token_uses_fixture.py` (revised: token-missing in live mode is a clean non-zero exit, not fallback; the test asserts the clean failure and asserts that `--offline` uses the fixture), `tests/test_dependency_overlay.py`.
...
ubu-phase0-demo/
  ...
  fixtures/
    static_dummy_issue.json              # canonical: test fixture + offline corpus + fallback
    demo_request.json
    demo_log_empty.json                  # []
    claim_register.json
  ...
  tests/
    ...
    test_missing_github_token_uses_fixture.py
